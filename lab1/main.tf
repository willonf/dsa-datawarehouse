...
resource "google_bigquery_table" "dsa_table_1" {
  ...
  dataset_id          = google_bigquery_dataset.dsa_dataset.dataset_id  # ou dsa_dataset
...
resource "google_bigquery_job" "job_sql_1" {
  ...
  load {
    source_uris = [
      "gs://dsa-dw-alpha/tbClienteDSA.csv",
    ]
...
resource "google_bigquery_job" "job_sql_2" {
  ...
  load {
    source_uris = [
      "gs://dsa-dw-alpha/tbProdutoDSA.csv",
    ]
